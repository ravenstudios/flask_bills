...
def add_new_bill_form():
    if request.method == 'GET':
        ...
        if id:
            return render_template('bill/bill-form.html', paychecks=Paycheck.query.all(), paycheck_from_from=Paycheck.query.get(id), today=datetime.datetime.now().strftime("%Y-%m-%d"))
        else:
